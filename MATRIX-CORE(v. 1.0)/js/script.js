...
//----------MATRIX CREATE STARTING--------------//
	var sizeX = 9; //small 9; default value 17;  large 33;
	var sizeY = 9; //small 9; default value 16;  large 33;
		...
		var cellNumber = 1; //create LOCAL var unique key for create unique key for the cell
			for (var y = 1; y-1<sY; y++){
				for (var x = 1; x-1<sX; x++){
					$("#matrix").append('<div class="matrix-cell" data-x-y='+x+'-'+y+' data-cell-value="0" data-cell-number='+cellNumber+' data-cell-content = "0">'+x+':'+y+'</div>');
					cellNumber++;  
				}
				$("#matrix").append('<br/>');
			}
...
//----------------MATRIX ANALYZER STARTING-----------------//
	function matrixAnalyzer(){
		...
		$(""+className+"").click(function(){
			...
			x = parseFloat($(this).data('x-y')[0]);
			y = parseFloat($(this).data('x-y')[2]);
...
//-----------------------ENDING----------------------//
	...
	function actionBorders (x,y){
		var arr = new Array(); 
		arr[0] = dataSample(className,"data-x-y",(x)+"-"+(y)).data('cell-number');
		arr[1] = dataSample(className,"data-x-y",(x-1)+"-"+(y-1)).data('cell-number');
		arr[2] = dataSample(className,"data-x-y",(x)+"-"+(y-1)).data('cell-number');
		arr[3] = dataSample(className,"data-x-y",(x+1)+"-"+(y-1)).data('cell-number');
		arr[4] = dataSample(className,"data-x-y",(x-1)+"-"+(y)).data('cell-number');
		arr[5] = dataSample(className,"data-x-y",(x+1)+"-"+(y)).data('cell-number');
		arr[6] = dataSample(className,"data-x-y",(x-1)+"-"+(y+1)).data('cell-number');
		arr[7] = dataSample(className,"data-x-y",(x)+"-"+(y+1)).data('cell-number');
		arr[8] = dataSample(className,"data-x-y",(x+1)+"-"+(y+1)).data('cell-number');	
